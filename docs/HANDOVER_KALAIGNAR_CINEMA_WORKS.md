...
- English translation: **scene 35/63 / 889 verified units**;
- English dialogue coverage: **746 explicit + 14 source-role supplements = 760 dialogue units**;
- English stage/action units: **127**;
- English song-reference units: **2**;
- English cross-page units: **21**;
...
Batch 31–35 source safeguards:

- scene 32 is action-only and creates no invented speech;
- five cross-page immutable records remain single logical owners: scene 33 d020 and scene 35 d014/d034/d074/d087;
- scene 33 preserves its conscience/worm/peacock metaphor, mixed-content source ownership and only the printed Layla–Majnun/Qays reference;
- scene 34 leaves frozen `வாட்டமா நான்?` visible by transliteration rather than speculative source repair;
- scene 35 links `ammaiyappan-s035-r001` as source-context-attributed Tirisangu provenance without creating a printed label;
- scene 35 preserves exact `முத்` / `திரி` source ownership, source irregularities, `oppari`, `Ardhanarishvaras`, the retail/wholesale joke, cross-dressing, mistaken abduction, ring exchange and Rama/Sita/Lakshmana forest-life humour;
- the closed song/performance gate has **0 retained occurrences in scenes 31–35**.

**Exact next activity:** translate and source-review archival scenes **36–40** from frozen verified derivatives. Inspect the live closed source-role layer and `songs/inventory.json` before writing. Scene **40** contains a retained source-visible song/performance occurrence; represent only booklet-supported material, keep exact speaker/source-role/page provenance and whole cross-page units, and do not modify frozen source evidence.