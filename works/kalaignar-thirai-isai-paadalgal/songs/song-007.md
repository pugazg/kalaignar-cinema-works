# 007 — பேசும் யாழே பெண் மானே

<!-- source: pdf=36 printed=36 anthology_song=007 status=verified -->

- film: `நாம்`
- anthology film year: `1953`
- music as printed: `சிதம்பரம் எஸ்.ஜெயராமன்`
- voice as printed: `ஜிக்கி (சோகம்)`
- source role label: `மீனா`
- attribution status: `anthology-attributed`
- transcription status: `verified`

## மூலம்

மீனா:

பேசும் யாழே பெண் மானே  
வீசும் தென்றல் நீதானே  
கேளேனோ இனிமேல் இதுபோல் கீதம்...

(பேசும்)

தாளா தீச்சுழலில்  
தனியாய் வீழ்ந்தீரோ  
மீளா சோகம் மீட்டிடும் யாழானேன்  
மாளா காதலினால் வாழ்ந்தோமே காதல்...

(பேசும்)

பாடும் பேயாக பாரினில் அலைவீரோ  
தீரா துன்பம் தீட்டிய ஏடானேன்  
மாறா பாசம்தனை மறந்தீரோ நேசம்

(பேசும்)

## Source / editorial notes

- PDF 32 lists a second `பேசும் யாழே பெண் மானே` item beside `கலைஞர்`; PDF 36 prints it as numbered lyric `007` with `ஜிக்கி (சோகம்)`.
- The repeated `(பேசும்)` cues and source wording are retained as printed.
- PDF 36 was visually rechecked before this record was marked `verified`.
