# 004 — மாறி மகமாயி மாறி

<!-- source: pdf=33 printed=33 anthology_song=004 status=verified -->

- film: `நாம்`
- anthology film year: `1953`
- music as printed: `சிதம்பரம் எஸ்.ஜெயராமன்`
- voice as printed: `செல்லமுத்து, ஏ.பி.கோமளா`
- attribution status: `anthology-attributed`
- transcription status: `verified`

## மூலம்

கோரஸ்

மாரி மகமாயி மாரி மகமாயி  
ஆயி எங்கள் காளியம்மா  
மாரி மகமாயி மாரி மகமாயி  
ஏழை நாங்கள் எல்லோரும் வாழ  
மாத மும்மாரி பொழியணும் தாயே..

ஆண்

(சூரியனால் கடல் நீரும் - மேகமாகவே  
மாறியதால் லோகத்திலே  
மழையும் பொழியுமா

பெண்

சொல்றேன் கேளு  
விஞ்ஞானம் புதிதாக மாறுது பாராய்  
கொடும்பாவி அருளாலே பெய்யுமாம் ஜோராய்  
மானம் காஞ்சா நாமெல்லோரும் வாடிடுவோம்  
தல்லாலே லா.... ....

எல்லோரும்

திரி சூலி நீலி எங்கள் மாரி மகமாயி  
வரி கொடுக்க முடியாமல்  
வாடுறோம் தாயே...  
ஓங்கார ரூபிணி மாரி மகமாயி  
எங்கள் குறை தீர்த்து விடு மாரி மகமாயி

(மாரி)

## Source / editorial notes

- PDF 32 lists the corresponding `மாரி மகமாயி மாரி மகமாயி` item beside `கலைஞர்` in the anthology's `நாம்` film-song list.
- PDF 33 is the numbered lyric page `004` and was visually rechecked before this record was marked `verified`.
- The source's colloquial forms, punctuation, role labels, open parenthesis before `சூரியனால்`, and repeated ellipses are preserved without normalization.
