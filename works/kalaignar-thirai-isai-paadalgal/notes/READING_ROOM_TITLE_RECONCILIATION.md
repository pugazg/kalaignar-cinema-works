...
- The audit, progress and work-readme documents are regenerated by
  `sync_status.py` from their next-activity heading to end of file;
  `PROJECT_HANDOVER.md` is regenerated between its next-activity heading and its
  repository-boundary heading. Durable hand-written history must stay outside
  those ranges — `notes/` is the safe home.
- `replace_section()` locates its boundaries with `text.find()`, so any prose
  that quotes a boundary heading verbatim ahead of the real heading will hijack
  the replacement. Describe those headings rather than reproducing them.
