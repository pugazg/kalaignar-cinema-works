...
def render_scene(scene_record: dict[str, Any]) -> str:
    ...
    parts = [f'<section class="scene" epub:type="chapter" xmlns:epub="http://www.idpf.org/2007/ops"><h1>Scene {scene}</h1>']
...
def build_entries(reader: dict[str, Any]) -> dict[str, bytes]:
    title_body = '''<section class="title-page" epub:type="titlepage" xmlns:epub="http://www.idpf.org/2007/ops"><h1>Tirumbippaar!</h1><p class="tamil-title" lang="ta">திரும்பிப்பார்!</p><p>English reader edition</p><p class="credit">Story &amp; dialogue credit as printed: Kalaignar M. Karunanidhi</p><p class="edition-note">Source-linked archival English derivative. Canonical Tamil remains authoritative.</p></section>'''
    nav_items = "\n".join(f'<li><a href="text/scene-{scene:03d}.xhtml">Scene {scene}</a></li>' for scene in EXPECTED_SCENES)
    nav = xhtml_document("Contents", f'''<nav epub:type="toc" id="toc" xmlns:epub="http://www.idpf.org/2007/ops"><h1>Contents</h1><ol>{nav_items}</ol></nav><nav epub:type="landmarks" hidden="hidden" xmlns:epub="http://www.idpf.org/2007/ops"><ol><li><a epub:type="bodymatter" href="text/scene-001.xhtml">Start of text</a></li></ol></nav>''').replace('href="../styles/book.css"', 'href="styles/book.css"')
    css = '''body { font-family: serif; line-height: 1.55; margin: 5%; } h1 { text-align: center; margin: 1.5em 0; } .title-page { text-align: center; margin-top: 20%; } .tamil-title { font-size: 1.5em; } .credit, .edition-note { margin-top: 2em; } .dialogue { margin: 0.75em 0; } .speaker { display: block; font-weight: bold; margin-bottom: 0.15em; } .stage { font-style: italic; margin: 0.8em 1em; } .special { border-left: 0.15em solid currentColor; margin: 1em; padding-left: 0.8em; } .unit-label { font-style: italic; font-weight: bold; margin-bottom: 0.2em; }'''
    manifest_items = [
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="pub-id" xml:lang="en"><metadata xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:identifier id="pub-id">urn:sha256:{SOURCE_SHA256}:tirumbippaar-en-reader</dc:identifier><dc:title>Tirumbippaar! — English Reader Edition</dc:title><dc:language>en</dc:language><dc:creator id="creator">Kalaignar M. Karunanidhi</dc:creator><meta refines="#creator" property="role" scheme="marc:relators">aut</meta><meta property="dcterms:modified">{MODIFIED}</meta><meta property="schema:accessMode">textual</meta></metadata><manifest>{' '.join(manifest_items)}</manifest><spine>{' '.join(spine_items)}</spine></package>\n'''
    container_xml = '''<?xml version="1.0" encoding="utf-8"?><container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container"><rootfiles><rootfile full-path="EPUB/package.opf" media-type="application/oebps-package+xml" /></rootfiles></container>\n'''
    ...
    for scene_record in reader["scenes"]:
        scene = scene_record["canonical_scene"]
        entries[f"EPUB/text/scene-{scene:03d}.xhtml"] = render_scene(scene_record).encode("utf-8")
    return entries
...
def write_epub(entries: dict[str, bytes]) -> None:
    with zipfile.ZipFile(EPUB_PATH, "w", compression=zipfile.ZIP_STORED, strict_timestamps=True) as zf:
        for name in ["mimetype"] + sorted(name for name in entries if name != "mimetype"):
...
def validate_epub(reader: dict[str, Any], entries: dict[str, bytes]) -> dict[str, Any]:
    ...
    ensure(EPUB_PATH.exists() and EPUB_PATH.stat().st_size > 0, "EPUB file was not created")
    ...
    with zipfile.ZipFile(EPUB_PATH, "r") as zf:
        ...
        ensure(zf.getinfo("mimetype").compress_type == zipfile.ZIP_STORED, "EPUB mimetype must be uncompressed")
        ensure(len(names) == expected_entry_count and set(names) == set(entries), "EPUB member set differs from deterministic build set")
        for name, expected in entries.items():
            ensure(zf.read(name) == expected, f"EPUB member bytes differ after packaging: {name}")
    all_units = [unit["id"] for scene in reader["scenes"] for unit in scene["units"]]
    all_text = [unit_text(unit) for scene in reader["scenes"] for unit in scene["units"]]
    ensure(len(all_units) == EXPECTED_UNITS == len(set(all_units)), f"Reader unit identity count is not exactly {EXPECTED_UNITS}")
    ...
    for uid in all_units:
        ensure(joined.count(f'data-unit-id="{uid}"') == 1, f"EPUB does not contain exactly one instance of {uid}")
    nav_text = entries["EPUB/nav.xhtml"].decode("utf-8")
    for scene in EXPECTED_SCENES:
        ensure(nav_text.count(f'<li><a href="text/scene-{scene:03d}.xhtml">Scene {scene}</a></li>') == 1, f"EPUB TOC mismatch for scene {scene}")
    ...
    return {"sha256":sha256_bytes(epub_bytes),"bytes":len(epub_bytes),"zip_members":expected_entry_count,"scene_documents":93,"unit_markers":EXPECTED_UNITS,"qa_status":"PASS"}


def main() -> int:
    reader = load_json(READER_JSON)
    ensure(reader.get("work_id") == "tirumbippaar" and reader.get("target_language") == "en", "Reader JSON identity mismatch")
    ...
    ensure(isinstance(scenes, list) and [record.get("canonical_scene") for record in scenes] == EXPECTED_SCENES, "Reader JSON scene records are incomplete/unordered")
...
**Format:** EPUB 3  
**Authority:** `reader-edition.json` generated from the corrected-source verified translation layer

## Verified checks

- canonical scenes packaged: **93/93**;
- verified English units packaged exactly once: **1,330/1,330**;
- reader kind checkpoint retained: **1,049 dialogue / 262 stage direction / 7 song-reference / 2 chant / 10 written-text / 0 full-song**;
- EPUB scene XHTML documents: **93**;
- package ZIP members: **{result['zip_members']}**;
- `mimetype` is first, exact and uncompressed;
- OPF manifest/spine and TOC contain all 93 scenes in corrected source order;
- every English unit ID appears in scene XHTML exactly once, regardless of historical numeric-ID order;
- no synthetic `(Scene ends.)` text is packaged;
- package uses fixed ZIP timestamps and uncompressed deterministic members.
...
"""
    ...
    package_manifest = {
        "work_id":"tirumbippaar","package":"english-reader-epub3","status":"complete-verified","source_scan_sha256":SOURCE_SHA256,"reader_authority":"works/tirumbippaar/editions/en/reader-edition.json","reader_sha256":sha256_bytes(READER_JSON.read_bytes()),"canonical_scenes":EXPECTED_SCENES,"translation_units":EXPECTED_UNITS,"unit_kind_counts":EXPECTED_KINDS,"epub":{"path":"tirumbippaar-en.epub",**result},"qa_report":"EPUB_QA_REPORT.md","deterministic_zip_timestamp":"1980-01-01T00:00:00","epub_modified_metadata":MODIFIED,
    }
