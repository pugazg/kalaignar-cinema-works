#!/usr/bin/env python3
"""Synchronize Ammayappan status after a passing deterministic English reader build."""
...
if manifest.get("status") != "complete-verified" or manifest.get("qa_status") != "PASS":
    raise SystemExit("Refusing status sync: Ammayappan reader manifest is not complete-verified/PASS")
if manifest.get("translation_units") != 1210 or manifest.get("dialogue_source_links_total") != 1025:
    raise SystemExit("Refusing status sync: Ammayappan reader checkpoint totals differ")
if len(manifest.get("cross_page_units", [])) != 28 or manifest.get("occurrence_source_span_links_total") != 7:
    raise SystemExit("Refusing status sync: Ammayappan cross-page/occurrence totals differ")
...
def replace_all_current(path: Path, old: str, new: str) -> bool:
    text = path.read_text(encoding="utf-8")
    if old not in text:
        if new in text:
            return False
        raise SystemExit(f"expected current text not found in {path.relative_to(ROOT)}: {old!r}")
...
def do(path: str, old: str, new: str, all_occurrences: bool = False) -> None:
    p = ROOT / path
    result = replace_all_current(p, old, new) if all_occurrences else replace_once(p, old, new)
    if result and path not in changed:
        changed.append(path)


# Translation index: close reader/export gate while preserving completed translation authority.
...
index_updates = {
    ...
    "next_activity": "Create and QA the source-linked Ammayappan Reading Room integration payload from the complete-verified reader/translation structures; preserve archive-only scene numbering, exact source provenance, dialogue/source-role distinctions and the five occurrence identities without reconstructing absent lyrics or authorship.",
}
index_changed = False
for key, value in index_updates.items():
...
# Work metadata.
metadata_fields = f'''  reader_export: complete-verified
...
work_reader_section = f'''## Reader/export package — PASS

The deterministic English reader/export package is now **complete-verified** under `editions/en/`. `reader-edition.md`, `reader-edition.html` and `reader-edition.json` each preserve all **1,210** verified units exactly once. Generated-output QA confirms all **1,025/1,025 dialogue/source-role links exactly once**, all **28** cross-page logical units, and all **5** occurrence identities across **7** intentional source-span links. Exact Tamil speaker-label/source-role provenance is preserved: printed colon labels remain printed-label provenance, scene 3 / scene 5 retain source-semicolon provenance, and context-attributed supplements are visibly marked as contextual rather than printed labels.
...
`editions/en/manifest.json` records authoritative-input aggregate hashes, output hashes and the PASS disposition. Reader generation changed **no** canonical Tamil, scene, dialogue/source-role, character or song/performance evidence.
...
# Translation README.
translation_reader_section = f'''## Reader/export package — PASS
...
root_reader_paragraph = f'''The **Ammayappan English reader/export package now passes generated-output QA**. Markdown, standalone HTML and machine-readable JSON each preserve all 1,210 verified units exactly once, all 1,025 dialogue/source-role links, all 28 cross-page units and all five occurrence identities across seven intentional source-span links. The integrity manifest records deterministic authoritative-input and output hashes. Output SHA-256 values are `{MD_SHA}` (Markdown), `{HTML_SHA}` (HTML) and `{JSON_SHA}` (JSON). No frozen Tamil or structured source evidence was modified.
...
do(
    ...
    root_reader_paragraph,
)

# Registry without reformatting unrelated work objects.
registry_path = ROOT / "data" / "works.json"
registry = registry_path.read_text(encoding="utf-8")
old_registry_tail = '"reader_export":"ready-after-preflight","reading_room_integration":"blocked-pending-reader-export-generation-and-qa","next_structured_derivative":"english-reader-export"}'
new_registry_tail = (
    '"reader_export":"complete-verified",'
    '"english_reader_edition_path":"works/ammaiyappan/editions/en",'
    '"english_reader_build_script_path":"works/ammaiyappan/editions/en/build.py",'
    '"english_reader_qa_report_path":"works/ammaiyappan/editions/en/QA_REPORT.md",'
    '"english_reader_manifest_path":"works/ammaiyappan/editions/en/manifest.json",'
    f'"english_reader_markdown_sha256":"{MD_SHA}",'
    f'"english_reader_html_sha256":"{HTML_SHA}",'
    f'"english_reader_json_sha256":"{JSON_SHA}",'
    '"english_reader_translation_units":1210,'
    '"english_reader_dialogue_source_links":1025,'
    '"english_reader_cross_page_units":28,'
    '"english_reader_occurrence_identities":5,'
...
master_reader = f'''### Reader/export package — PASS

`works/ammaiyappan/editions/en/` now contains deterministic Markdown, standalone HTML and machine-readable JSON plus `QA_REPORT.md` and `manifest.json`. Generated-output QA confirms all **1,210** verified units exactly once in each export, all **1,025** dialogue/source-role links, all **28** cross-page units and all **5 occurrence identities / 7 source-span links**. Output SHA-256 values are `{MD_SHA}` (Markdown), `{HTML_SHA}` (HTML) and `{JSON_SHA}` (JSON). No frozen source evidence was modified.
...
project_reader = f'''## Reader/export package — PASS
...
# Next-chat prompt: add reader outputs to startup and move exact activity to Reading Room payload.
do(
